...
<body>
	<jsp:include page="backnav.jsp">
      ...
      <div class="header">
        <div class="now color"><a href="../../back/index" class="color">后台</a> -> 公司新闻</div>
      </div>
        ...
        <div class="listbox">
            ...
            <div class="tablemain">
                <table width="100%">
                  <tr> 
                    ...
                    <th width="23%" scope="col">新闻时间</th>
                    <th width="23%" scope="col">添加时间</th>
		   ...
		   <%inString = request.getAttribute("intro") ;%>
                  <tr>
                    ...
                    <td>${item.title}</td>
                    <td> <fmt:formatDate value="${item.newstime}" pattern="yyyy/MM/dd"/>
					</td>
                    <td><jsp:useBean id="dateValue" class="java.util.Date"/>
                    <jsp:setProperty name="dateValue" property="time" value="${item.cid}"/>
                    <fmt:formatDate value="${dateValue}" pattern="yyyy/MM/dd HH:mm:ss"/></td>
                    <td>
                    	<input type="button" value="编辑" class="bottons c_qing" onclick="location.href='update?id=${item.cid}'">
                        <input type="button" value="删除" class="bottons c_red" onClick="alert_del(${item.cid})">
                    </td>
                  </tr>
                 </c:forEach>
