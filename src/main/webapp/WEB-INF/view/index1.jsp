...
<div id="qsq" class="qsq">
				...
				<ul>
					...
					<c:set var="intro" value="${item.photo }" scope="request"/>
					 ...
					 <%inString = request.getAttribute("intro") ;%>
						<li><a href="http://${item.website }"><img src="<%=showPhoto.getPhoto(byteToStr.objToStr(inString))%>" /></a>
						</li>
			 		</c:forEach>
				</ul>
			</div>
	<!-- 下面是前/后按钮代码，如果不需要删除即可 -->
		...
		jQuery(".qsq").slide({mainCell:".bd ul",effect:"left",autoPlay:true,delayTime:1000});
		</script>
