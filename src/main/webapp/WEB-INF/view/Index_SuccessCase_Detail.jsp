...
<body>		 <jsp:include page="backnav.jsp">
      ...
      <div class="header">
        <div class="now color"><a href="../../back/index" class="color">后台</a> -> 网站首页 -> 成功案例</div>
      </div>
      
      <div class="logomain">
      <a class="style1">添加成功案例</a>
      <div class="sform3">
     <ul>
              <form action="<c:if test="${state.equals('save')}">save</c:if>
		<c:if test="${state.equals('update')}">update?id=${succeful.sid}</c:if>" class="container" enctype="multipart/form-data" method="post" id='sysForm'>
              <li>
                <div class="fll1">标题<strong class="red">*　　</strong></div>
                <div class="gill"><input  class="contactinputs" name="title" value="${succeful.title }" type="text" id="title">
                </div>
              </li>
              <li>
